...
class Terminal extends Component {
    ...
    keyModifierHandler(e) {
        ...
        let length = newContent.length;
        if (e.key === 'Backspace') {
            newContent[length - 1] = newContent[length - 1].slice(
                0,
                newContent[length - 1].length - 1
            );
        }
        this.setState({
            content: newContent,
    ...
    executeCommand() {
        const { menuOpen, history, toggleMenu } = this.props;
        let newContent = this.state.content.slice();
        ...
        if (keywords[0] === 'cd') {
            ...
            } else if (keywords[1] === 'ShopifyChallenge') {
                history.push('/ShopifyChallenge');
                newContent.push('');
                if (menuOpen) {
                    toggleMenu();
                }
            ...
            newContent.push(
                'output Home About Projects Experience ShopifyChallenge'
            );
            newContent.push('');
            ...
            newContent.push('');
            window.open(
                'https://www.linkedin.com/in/nuri-amari-a22b21144/',
                '_blank'
            );
    ...
    render() {
        return (
            <div
                tabIndex="0"
                onKeyDown={this.keyModifierHandler}
                onKeyPress={this.keyHandler}
                className={'Terminal ' + this.props.passedClass}
            >
